...
import { Button, ButtonGroup, Input, InputGroup, InputGroupButton, Row, Col } from 'reactstrap';
...
class CreateGame extends Component {
	...
	render() {
		const playerNames = Array(this.state.numPlayers)
			.fill('player').map((p,i)=>p+(i+1))
			.map(p=>this.state[p]);

		const everyoneHasNames = playerNames.every(playerName=>playerName.length > 0);
		const dupNames = [...new Set(playerNames)].length !== playerNames.length;

		const localColor = this.state.gameType === 'local' ? 'success' : 'secondary';
		const onlineColor = this.state.gameType === 'online' ? 'success' : 'secondary';
		const numPlayers = Array(4).fill('').map((a,i)=>
			<Button key={i}
		...
		const playerNamesEl = Array(this.state.numPlayers).fill('').map((a,i)=>
			<Row key={i}><Col xs={{ size : 10, offset : 1}}>
				<InputGroup>
					{
						this.state['player'+(i+1)].length > 0 &&
						playerNames.filter(n=>n===this.state['player'+(i+1)]).length > 1 &&
						<InputGroupButton color="danger">Duplicate Name</InputGroupButton>
					}
					<Input player={i}
		...
		return (
			<div>
				...
				<br />
				{
					!dupNames &&
					everyoneHasNames &&
					<Button onClick={this.createGame.bind(this)} color="success">Start Game</Button>
				}
				{!everyoneHasNames && <Button color="danger">Give Each Player a Name</Button>}
			</div>
		);
	}
}
